...
#ifdef LORA_HF_BOARD		
#else
			obj->Adc.Init.DiscontinuousConvMode = ENABLE;			
#endif			
    }
    else
    {
...
#else
        obj->Adc.Init.ExternalTrigConv = ADC_SOFTWARE_START;
...
#else
        obj->Adc.Init.ScanConvMode = ADC_SCAN_ENABLE; 
        obj->Adc.Init.NbrOfConversion = 3;
        //AdcHandle.Init.NbrOfConversion       = 3;     /* Sequencer of regular group will convert the 3 first ranks: rank1, rank2, rank3 */
        obj->Adc.Init.NbrOfDiscConversion   = 1;    /* Sequencer of regular group will convert ranks one by one, at each conversion trig */   			
#endif
