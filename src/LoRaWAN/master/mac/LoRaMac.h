...
/*!
 * User definition  by junhua
 */
typedef enum eLoraMode{
   LORAWAN_MODE = 0,
   LORAP2P_MODE 
}E_LORA_MODE;
...
typedef struct sLoRaMacCallback
{
     ...
     */
    void    ( *P2PRxTimeout )( void );
    /*!
     * \brief Rx Error callback prototype.
     */
    void    ( *P2PRxError )( void );
...
void LoRaSetP2PTxCallback( void( *P2PTxDone )( void ));
void LoRaClrRadioStatus(void);
RadioStatus_t* LoRaGetRadioStatus(void);
void SX127X_INIT(void);
LoRaMacStatus_t LoRaMacChannelSwitch( uint8_t id, int status, ChannelParams_t params );
